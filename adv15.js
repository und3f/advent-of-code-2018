        ...
        for (let j = 0; j < this.width; j++)
            switch (boardString[i][j]) {
                ...
                case '.':
                    break;
                case 'G':
                    unit = Object.assign({race: 'G', position: [i, j]}, this.unitObject);
                    this.goblins.push(unit);
                    this.board[i][j] = unit;
                    break;
                case 'E':
                    unit = Object.assign({race: 'E', position: [i, j]}, this.elveUnitObject);
...
Game.prototype.findAdjancedEmptyPoints = function (_position, outputArray) {
    ...
    for (let d in directions) {
        ...
        if (position && this.board[position[0]][position[1]] == null)
            outputArray.push(position);
    }
...
Game.prototype.unitMove = function(unit) {
    ...
    let reachable = inRange.map(e => [...e, distancies[e[0]][e[1]]]).filter(e => e[2] !== null).sort((a, b) => a[2] - b[2]);
    ...
    for (let i in directions) {
        let d = directions[i];
        if (backwardDistancies[d[0]][d[1]] < backwardDistancies[bestDirection[0]][bestDirection[1]])
            bestDirection = d;
    }
...
Game.prototype.unitAttack = function(unit) {
    ...
    for (let i = 1; i < enemies.length; i++) {
        if (enemies[i].hp < choosedEnemy.hp)
            choosedEnemy = enemies[i];
    }
...
Game.prototype.showUnits = function() {
    let units = [...this.elves, ...this.goblins].sort((a, b) => (a.position[0] - b.position[0]) || (a.position[1] - b.position[1]));
...
Game.prototype.execute = function() {
    ...
    do {
        ...
        console.log("Round ", game.rounds);
            console.log(game.showBoard()); console.log(game.showUnits());
